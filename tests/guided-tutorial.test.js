...
test('shows the auto-reveal lesson once, only in intermediate after a new flag completes a clue', () => {
  ...
  assert.match(appSource, /refreshMediumChordObjectiveTarget\(snapshot = this\.roomSnapshot\)/);
  assert.match(appSource, /const exactTarget = chordOpportunityAt\(snapshot, this\.mediumChordObjectiveTarget\)/);
  assert.match(appSource, /const nextTarget = exactTarget \?\? findChordOpportunity\(snapshot\)/);
  assert.match(appSource, /updateGridVisibility\(\);\s*this\.refreshMediumChordObjectiveTarget\(\);\s*this\.updateMissionGuide\(\)/);
  assert.match(appSource, /this\.mediumChordObjectiveActive = false;\s*this\.setMediumChordObjectiveTarget\(null\)/);
  assert.match(appSource, /ensureMediumChordObjectiveMarker\(\)[\s\S]*marker\.name = 'medium-chord-objective-marker'/);
  assert.match(appSource, /setMediumChordObjectiveTarget\(target\)[\s\S]*this\.ensureMediumChordObjectiveMarker\(\)/);
  assert.match(appSource, /cell\.spriteInstance = sprite;[\s\S]*this\.ensureMediumChordObjectiveMarker\(\)/);
  assert.match(appSource, /mediumChordObjectivePending[\s\S]*Boolean\(this\.mediumChordObjectiveTarget\)/);
  ...
  assert.match(appSource, /tutorial\.actionHint\.\$\{displayAction\}\$\{outsideSlice \? '\.outsideSlice' : ''\}/);
  assert.match(i18nSource, /电脑把指针放在金色数字上，同时按下左右键/);
  assert.match(i18nSource, /单击未开启格继续加旗，双击已开启数字自动展开/);
  assert.match(i18nSource, /'task\.medium\.guide\.chordOne': '对金色圈出的数字 \{number\} 执行一次自动展开'/);
  assert.match(i18nSource, /'task\.medium\.guide\.chordOneOutsideSlice': '自动展开目标在当前切片外：数字 \{number\}'/);
  assert.match(i18nSource, /'tutorial\.actionHint\.chord': '中级实操：金色圈出的数字 \{number\} 周围旗子已经标够，而且仍有邻格可开。[^']*同时按下左右键[^']*双击金色数字/);
  assert.match(i18nSource, /'tutorial\.actionHint\.chord\.outsideSlice': '[^']*当前切片外[^']*显示全部/);
});
