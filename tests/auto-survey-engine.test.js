...
function createSolo(config, random = seededRandom()) {
  const engine = RoomEngine.create({
    code: 'MAPS99', hostId: 'host', hostName: 'Host', tokenHash: 'hash', mode: 'solo', now: 1_000,
  });
...
test('allows only a solo host to control automated survey and blocks manual board actions while running', () => {
  ...
  assert.throws(
    () => squad.apply('host', { op: 'auto_survey_start' }, { id: 'squad-host', sequence: 1, now: 1_200 }),
    /SOLO_ONLY/,
  );
  assert.throws(
    () => squad.apply('guest', { op: 'auto_survey_start' }, { id: 'squad-guest', sequence: 1, now: 1_300 }),
    /HOST_ONLY/,
  );

  const engine = createSolo(scanConfig);
  command(engine, 2, { op: 'auto_survey_start' });
...
test('uses the normal protected first dig and exposes no future target or hidden mine data', () => {
  ...
  assert.deepEqual(Object.keys(startSnapshot.autoSurvey).sort(), ['runId', 'startedBy', 'status', 'step', 'strategy']);
  assert.equal(startSnapshot.autoSurvey.strategy, 'scan');
  assert.equal(startSnapshot.autoSurvey.step, 0);
  assert.deepEqual(startSnapshot.mines, []);
  assert.equal(startSnapshot.replay, undefined);

  const runId = startSnapshot.autoSurvey.runId;
  command(engine, 3, { op: 'auto_survey_step', runId });
  ...
  assert.equal('target' in snapshot.autoSurvey, false);
  assert.equal('plan' in snapshot.autoSurvey, false);
  assert.equal('hasVisibleFlag' in snapshot.autoSurvey, false);
});
...
test('keeps a scan-strategy flag visible for one snapshot before a later dig may Auto-Purge it', () => {
  ...
  command(engine, 2, { op: 'auto_survey_start' });
  const runId = engine.state.autoSurvey.runId;
  ...
  command(engine, 4, { op: 'auto_survey_step', runId });
...
test('uses hidden truth for compression steps and finalizes the successful replay', () => {
  ...
  const snapshot = engine.snapshot();
  assert.equal(snapshot.phase, 'won');
  assert.equal(snapshot.autoSurvey.strategy, 'compression');
  assert.equal(snapshot.autoSurvey.status, 'completed');
  assert.equal(snapshot.autoSurvey.step, 1);
...
test('cancels by run id, rejects stale pulls, permits manual play afterward, and restart clears status', () => {
  ...
  const runId = engine.state.autoSurvey.runId;
  assert.throws(
    () => command(engine, 3, { op: 'auto_survey_cancel', runId: 'stale-run' }),
    /STALE_AUTO_SURVEY/,
  );

  command(engine, 4, { op: 'auto_survey_cancel', runId });
  assert.equal(engine.snapshot().autoSurvey.status, 'cancelled');
  assert.throws(
    () => command(engine, 5, { op: 'auto_survey_step', runId }),
    /WRONG_PHASE/,
  );
  ...
  assert.equal(engine.snapshot().autoSurvey, null);
});
...
test('serializes and restores an active pull session without exposing its private control fields', () => {
  ...
  assert.equal(restored.state.autoSurvey.runId, runId);
  assert.equal(restored.state.autoSurvey.status, 'running');
  assert.equal(restored.state.autoSurvey.step, 1);
  assert.deepEqual(Object.keys(restored.snapshot().autoSurvey).sort(), ['runId', 'startedBy', 'status', 'step', 'strategy']);
  command(restored, 4, { op: 'auto_survey_step', runId });
  assert.equal(restored.state.autoSurvey.step, 2);

  const legacy = restored.serialize();
  delete legacy.autoSurvey;
  assert.equal(RoomEngine.restore(legacy).snapshot().autoSurvey, null);
});

test('de-duplicates the same observed automated-survey step across distinct transport commands', () => {
  const engine = createSolo(compressionConfig);
  command(engine, 2, { op: 'auto_survey_start' });
  const runId = engine.state.autoSurvey.runId;

  const first = command(engine, 3, { op: 'auto_survey_step', runId, expectedStep: 0 });
  assert.equal(first.kind, 'applied');
  assert.equal(engine.snapshot().autoSurvey.step, 1);

  const retryWithNewCommandId = command(engine, 4, {
    op: 'auto_survey_step', runId, expectedStep: 0,
  });
  assert.equal(retryWithNewCommandId.kind, 'applied');
  assert.equal(retryWithNewCommandId.snapshot.autoSurvey.step, 1);

  command(engine, 5, { op: 'auto_survey_step', runId, expectedStep: 1 });
  assert.equal(engine.snapshot().autoSurvey.step, 2);
});
...
for (const { label, config, expectedStrategy, seed } of [
  ...
  test(`authoritatively completes a 9x9x9 ${label} board within one-cell progress bounds`, () => {
    ...
    while (engine.state.phase !== 'won' && sequence <= 732) {
      command(engine, sequence, { op: 'auto_survey_step', runId }, 'host', 3_000 + sequence);
    ...
    assert.equal(snapshot.phase, 'won');
    assert.equal(snapshot.autoSurvey.status, 'completed');
    assert.equal(snapshot.autoSurvey.strategy, expectedStrategy);
    assert.ok(snapshot.autoSurvey.step <= 730);
    assert.equal(snapshot.replay.steps.length, snapshot.autoSurvey.step);
