...
let fallbackRuntimeIdCounter = 0;

function fillFallbackRuntimeIdBytes(bytes) {
  fallbackRuntimeIdCounter = (fallbackRuntimeIdCounter + 1) >>> 0;
  const now = Date.now();
  for (let index = 0; index < bytes.length; index += 1) {
    const timeByte = Math.floor(now / (2 ** ((index % 6) * 8))) & 0xff;
    const counterByte = (fallbackRuntimeIdCounter >>> ((index % 4) * 8)) & 0xff;
    bytes[index] = Math.floor(Math.random() * 256) ^ timeByte ^ counterByte;
  }
}

// `crypto.randomUUID()` is deliberately unavailable on some insecure HTTP
// origins (notably Safari on a LAN IP). Event and replay identifiers are not
// authentication secrets, so preserve UUID-shaped IDs with getRandomValues(),
// and keep a final in-memory fallback for older/private browsing runtimes.
export function createRuntimeId(cryptoObject = globalThis.crypto) {
  ...
  if (typeof cryptoObject?.getRandomValues === "function") {
    try { cryptoObject.getRandomValues(bytes); } catch { fillFallbackRuntimeIdBytes(bytes); }
  } else {
    fillFallbackRuntimeIdBytes(bytes);
  }
...
// Room state contains JSON-compatible data only. The fallback lets local solo
// saves work on Safari versions that predate structuredClone().
...
function restoreReplayDraft(value, config) {
  ...
  return {
    version: 1,
    runId: typeof value.runId === "string" && value.runId ? value.runId : createRuntimeId(),
    config: { ...config },
...
export class RoomEngine {
  ...
  finalizeReplay(now) {
    ...
    this.state.completedReplay = {
      ...
      completedAt: now,
      steps: cloneGameState(draft.steps),
    };
  ...
  chat(playerId, content, now) {
    ...
    this.state.chat.push({ id: createRuntimeId(), playerId, playerName: this.member(playerId).name, message: text, at: now });
  ...
  addActivity(key, params, now) {
    this.state.activity.push({ id: createRuntimeId(), key, params, at: now });
